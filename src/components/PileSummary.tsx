            ...
            >
              ...
              <div className="mt-3 pt-3 border-t border-white/30">
                <div className="grid grid-cols-6 gap-1 text-center">
                  {/* Row 1: Empty label + Letter squares */}
                  <div />
                  {SUB_LABELS.map((label, i) => (
                    <div key={label} className="flex justify-center">
                      <div className={`w-6 h-6 rounded ${colors.subDots[i]} flex items-center justify-center`}>
                        <span className={`text-[13px] font-bold ${i >= 3 ? 'text-white' : colors.text}`}>{label}</span>
                      </div>
                    </div>
                  ))}

                  {/* Row 2: GCV label + individual GCV values */}
                  <div className="flex items-center justify-center">
                    <span className="text-[13px] uppercase font-semibold text-slate-400 tracking-wide">GCV</span>
                  </div>
                  {SUB_LABELS.map((label, i) => {
                    ...
                    return (
                      <div key={label}>
                        <span className={`text-[13px] font-medium ${hasData ? colors.text : 'text-slate-400'}`}>
                  ...
                  {/* Row 3: QTY label + individual Quantity values */}
                  <div className="flex items-center justify-center">
                    <span className="text-[13px] uppercase font-semibold text-slate-400 tracking-wide">QTY</span>
                  </div>
                  {SUB_LABELS.map((label, i) => {
                    const lotIndex = (stat.pile - 1) * 5 + i;
                    const lot = lots[lotIndex];
